### rais function masterlist ### 
## this file contains all minor functions used in the different clean functions.
# it is organized by variable with functions that are different between years having different year suffixes
...
### natjuridica ----
natjuridica_rais_2005 <- function(rais_data) {
  rais_data %>%
...
natjuridica_rais_2012 <- function(rais_data) {
  rais_data %>%
  mutate(natjuridica = as.numeric(natjuridica))%>%
  mutate(natjuridica = fct_recode(as_factor(natjuridica ),
                                  "POD EXEC FED" =  "1015",
  ...
  natjuridica_rais_2013 <- function(rais_data) {
    rais_data %>%
      mutate(natjuridica = as.numeric(natjuridica))%>%
      mutate(natjuridica = fct_recode(as_factor(natjuridica ),
                                      "POD EXEC FED" =  "1015",
...
### reorder ----
reorder_rais_2005 <- function(rais_data) {
  rais_data %>%
...
reorder_rais_2012 <- function(rais_data) {
  rais_data %>% 
    relocate(PIS, CPF, numectps, nome, identificad, radiccnpj, municipio,
...
#weird names are included that are not in the dataset
reorder_rais_2013 <- function(rais_data) {
  rais_data %>% 
    relocate(PIS, CPF, numectps, nome, identificad, radiccnpj, municipio,
           tpvinculo, empem3112, tipoadm, dtadmissao, causadesli, mesdesli,
           ocup2002, grinstrucao, genero, idade, nacionalidad, portdefic, tpdefic, raca_cor, 
