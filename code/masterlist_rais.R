...
rename_rais_2016 <- function(rais_data) {
  newnames <- newnames_rais(rais_data)
  setnames(rais_data, new = newnames ) # replacing old names with new
  rais_data %>% 
    clean_names() %>% 
...
destring_rais_2003 <- function(rais_data) {
  var_need_destringing <- c("municipio", "tipoadm", "tpvinculo", "causadesli","diadesli", "empem3112", "mesdesli", "grinstrucao", "tamestab", "tipoestbl","natjuridica", "horascontr", "indceivinc", "tiposal", "indalvara", "indpat", "indsimples", "portdefic", "raca_cor")
  rais_data %>%
    mutate_at(var_need_destringing, as.numeric)
}
destring_rais_2004 <- function(rais_data) {
  var_need_destringing <- c("municipio", "tipoadm", "tpvinculo", "causadesli","diadesli", "empem3112", "mesdesli", "grinstrucao", "tamestab", "tipoestbl", "horascontr", "indceivinc", "tiposal", "indalvara", "indpat", "indsimples", "portdefic", "raca_cor")
  rais_data %>%
    mutate_at(var_need_destringing, as.numeric)
}
...
destring_rais_2018 <- function(rais_data) {
  var_need_destringing <- c("municipio", "tipoadm", "tpvinculo", "causadesli", "empem3112", "mesdesli", "diadesli","idade", "grinstrucao", "sexotrabalhador", "tamestab", "tipoestbl", "horascontr", "indceivinc", "tiposal", "indalvara", "indpat", "indsimples", "indtrabparc", "indtrabint", "portdefic", "tpdefic", "raca_cor", "qtdiasafas", "causafast1","causafast2","causafast3", "anochegbr", "ibgesubsetor", "muntrab",
                            "clascnae95"  )
  rais_data %>%
    mutate_at(var_need_destringing, as.numeric)
}
...
commareplace_rais_2016 <- function(rais_data) {
  var_need_subbing <- c("remdezembro", "remmedia", "remdezr", "remmedr", "tempempr", "salcontr", "ultrem","remjan","remfev","remmar","remabr","remmai","remjun","remjul","remago","remset","remout"
  )
  rais_data %>%
    mutate_at(var_need_subbing, ~ str_replace_all(. ,"," ,".")) %>% # changing commas to
    # periods and then shifting to numeric
...
natjuridica_rais <- function(rais_data) {
  rais_data %>%
    ...
    mutate(natjuridica = fct_recode(as_factor(natjuridica ),
                                    ...
                                    "FUNDO PUBLIC" = "1201",
                                    "ASSOC PUBLIC" = "1210",
                                    "Consórcio Público de Direito Privado" = "1228",
                                    "Estado ou Distrito Federal" = "1236",
                                    "Município" = "1244",
                                    "Fundação Pública de Direito Privado Federal" = "1252", 
                                    "Fundação Pública de Direito Privado Estadual ou do Distrito Federal" = "1260",
