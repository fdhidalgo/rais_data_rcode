clean_rais_2012 <- function(rais) {
  ...
  setnames(rais, new = newnames ) # replacing old names with new
  ...
  var_need_destringing <- c("municipio", "tipoadm", "tpvinculo", "causadesli", "empem3112", "mesdesli", "idade", "grinstrucao", "sexotrabalhador", "tamestab", "tipoestbl", "horascontr", "indceivinc", "tiposal", "indalvara", "indpat", "indsimples", "portdefic", "tpdefic", "raca_cor", "qtdiasafas", "causafast1","causafast2", "causafast3", "clascnae95")
  ...
  lazy <- lazy %>% 
    mutate_at(var_need_destringing, as.numeric) %>%  # destringing some variable
    mutate_at(c("PIS","CPF","nome" ),str_trim) 
  ...
  lazy <- lazy %>%  
    ...
    mutate(identificad = case_when(nchar(identificad) == 3 ~ paste0("00000000000",identificad),
                                   ...
                                   identificad == 0 ~ "", 
                                   TRUE ~ as.character(identificad) 
    ))  # the above changes identificad
  
  lazy <- lazy %>% # recode 0 to blank
  ...
  lazy <- lazy %>% 
    ...
    mutate(ocup2002 = if_else(is.na(ocup2002), 0, ocup2002)) # above is recoding of ocup2002
  ...
  lazy <- lazy %>%
    mutate(indalvara = fct_recode(as_factor(indalvara), "Nao" = "1" ,  "Sim" = "2"),
           ...
           indsimples = fct_recode(as_factor(indsimples), "Nao" = "0" ,  "Sim" = "1")) # above is recoding of indalvara, indpat, indsimples
  ...
  rais <- lazy %>%
    select(PIS, CPF, numectps, nome, identificad, radiccnpj, municipio, tpvinculo, empem3112, tipoadm, dtadmissao, causadesli, mesdesli, ocup2002, grinstrucao, genero, idade, nacionalidad, portdefic, tpdefic, raca_cor, sbclas20, clascnae95, clascnae20, tamestab, natjuridica, tipoestbl, remdezembro, remdezr, remmedia, remmedr, tempempr, tiposal, salcontr, ultrem, horascontr, indceivinc, ceivinc, indalvara, indpat, indsimples) %>%  #this reorders the
    # dataframe and drops a duplicate column made in the process
    
    as.tibble()  #moving back into a dataframe
