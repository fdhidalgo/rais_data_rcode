...
clean_rais <- function(rais) {
  ...
  lazy <- lazy %>%
   ...
   rais <- lazy %>%
    select(PIS, CPF, numectps, nome, identificad, radiccnpj, municipio, tpvinculo, empem3112, tipoadm, dtadmissao, causadesli, diadesli, mesdesli, ocupacao94,  ocup2002,  grinstrucao, genero, dtnascimento, nacionalidad, portdefic, raca_cor, remdezembro, remdezr, remmedia, remmedr, tempempr, tiposal, salcontr ,ultrem, horascontr, clascnae95, tamestab, natjuridica, tipoestbl, indceivinc, ceivinc, indalvara, indpat ,indsimples) %>%  #this reorders the
    # dataframe and drops a duplicate column made in the process
    
    as.data.frame()  #moving back into a dataframe
